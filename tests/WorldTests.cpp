...
TEST_CASE("World should add a gaussian noise to the executed move commands", "[random-test]") {
    ...
    double initialRobotPose = 2.0;
    double distance = 1.0;
    double meanAfterMove = 3.0;
    double accuracy = 10.0;
    double standardDeviation = 0.05;
    auto itemIsBelowOneStandardDeviationBelowMean = [meanAfterMove, standardDeviation](double item){ return item < meanAfterMove - standardDeviation; };
    auto itemIsInOneStandardDeviationBelowMean = [meanAfterMove, standardDeviation](double item){ return (item >= meanAfterMove - standardDeviation) && (item < meanAfterMove); };
    auto itemIsInOneStandardDeviationAboveMean = [meanAfterMove, standardDeviation](double item){ return (item >= meanAfterMove) && (item < meanAfterMove + standardDeviation); };
    auto itemIsAboveOneStandardDeviationAboveMean = [meanAfterMove, standardDeviation](double item){ return item >= meanAfterMove + standardDeviation; };
    Mock<IMovingObject> robot;
    When(Method(robot, getMoveCommandAccuracyInPercentage)).AlwaysReturn(accuracy);
